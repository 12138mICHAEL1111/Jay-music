  ...
  <el-container class="co1">
    ...
    <el-main>
      ...
      <el-row  type="flex" class="row-bg"  justify="center" align="middle" v-for="(item, i) in items"  :key="i"  style="margin-top: 20px" >
        <el-col :span="9">
          <div class="musicList">
            <span class="musictitle"> {{ item.name }} ——《 {{ item.album }} 》</span >
            <div class="inButton">
            ...
            <div class="MusicWidgets">
              <el-button  @click="previous"  circle icon="el-icon-back"  size="mini"
                style="
                  ...
                  font-size: 20px;
                " ></el-button>
              <el-button circle icon="el-icon-video-pause" size="mini"  style="
                  background-color: #333;
                  color: white;
                  border-width: 0px;
                  font-size: 20px;
...
export default {
  ...
  methods: {
    ...
    async fetch() {
      const res = await this.$http.post("/user/musicliked", {
        id: localStorage.id,
      });
      this.userToken = localStorage.userToken;
      this.items = res.data.likemusic;
